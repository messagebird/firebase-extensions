...
function initialize() {
    if (initialized === true)
        return;
    initialized = true;
    (0, log_1.logInfo)("initializing app...");
    admin.initializeApp();
...
function deliver(payload, ref) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        (0, log_1.logInfo)("delivery attempt");
        try {
            ...
            (0, log_1.logInfo)(`sending message to channelId: ${payload.channelId}`);
            (0, log_1.logInfo)(`with content:`, payload.content);
            yield new Promise((resolve, reject) => {
                mb.conversations.start(payload, function (err, response) {
                    if (err) {
                        (0, log_1.logWarn)(`send failed, got error: ${err}`);
                        return reject(err);
                    }
                    (0, log_1.logInfo)(`send successfully scheduled, got response: ${response}`);
            ...
            (0, log_1.logInfo)(`updating delivery record with error message`);
...
function processCreate(snap) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, log_1.logInfo)("new msg added, init delivery object for it");
        return db.runTransaction((transaction) => {
...
function processWrite(change) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, log_1.logInfo)("processing write");
        if (!change.after.exists) {
            (0, log_1.logInfo)("ignoring delete");
            return null;
        }
        if (!change.before.exists && change.after.exists) {
            (0, log_1.logInfo)("process create");
            return processCreate(change.after);
        }
        const payload = change.after.data();
        (0, log_1.logInfo)("processing update");
        if (!payload.delivery) {
            (0, log_1.logInfo)("ignoring empty delivery update");
            return null;
        }
        switch (payload.delivery.state) {
            case "SUCCESS":
            case "ERROR":
                (0, log_1.logInfo)("current state is SUCCESS/ERROR");
                return null;
            case "PROCESSING":
                (0, log_1.logInfo)("current state is PROCESSING");
            ...
            case "PENDING":
            case "RETRY":
                (0, log_1.logInfo)("current state is PENDING/RETRY");
                ...
                (0, log_1.logInfo)("record set to PROCESSING state, trying to deliver the message");
...
exports.processQueue = functions.firestore.document(config_1.default.msgCollection + '/{msgId}').onWrite((change) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        (0, log_1.logWarn)("unexpected error during execution: ", err);
        return null;
    }
}));
